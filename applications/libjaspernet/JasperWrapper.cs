...
public class Jasper
{
    ...
    {
        int retval = jas_init();
        bitmap_format_id = jas_image_strtofmt("bmp");
        Console.WriteLine("bmp = " + bitmap_format_id);
        return retval;
    }
    ...
    [DllImport(JASPER_LIBRARY)]
    public static extern int jas_stream_destroy(IntPtr stream);
    ...
    private static int bitmap_format_id = 0;
    ...
    public static byte[] jasper_decode_j2c_to_bmp(byte[] input, int output_buffer_size)
    {
        IntPtr input_stream_ptr = jas_stream_memopen(input);
        ...
        int retval = jas_image_encode(image_ptr, output_stream_ptr, bitmap_format_id, "");
        ...
        //jas_stream_destroy(output_stream_ptr);
        jas_stream_close(input_stream_ptr);
