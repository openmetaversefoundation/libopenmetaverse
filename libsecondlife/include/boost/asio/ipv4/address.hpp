...
#ifndef BOOST_ASIO_IPV4_ADDRESS_HPP
#define BOOST_ASIO_IPV4_ADDRESS_HPP
...
/**
 * The boost::asio::ipv4::address class provides the ability to use and
...
public:
  ...
  {
    if (boost::asio::detail::socket_ops::inet_pton(AF_INET, host, &addr_) <= 0)
    {
  ...
  {
    char addr_str[boost::asio::detail::max_addr_str_len];
    const char* addr =
      boost::asio::detail::socket_ops::inet_ntop(AF_INET, &addr_, addr_str,
          boost::asio::detail::max_addr_str_len);
    if (addr == 0)
    {
      boost::asio::error e(boost::asio::detail::socket_ops::get_error());
      boost::throw_exception(e);
...
#endif // BOOST_ASIO_IPV4_ADDRESS_HPP
