...
*/
using System;
namespace jj2000.j2k.util
{
	
	/// <summary> This class presents a collection of static methods that allow access to
	...
	/// 
	/// </summary>
	public sealed class NativeServices
	{
		//UPGRADE_NOTE: Respective javadoc comments were merged.  It should be changed in order to comply with .NET documentation conventions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1199'"
		/// <summary> Returns the current concurrency level. See 'setThreadConcurrency' for
		...
		/// 
		/// </returns>
		/// <seealso cref="setThreadConcurrency">
		/// 
		/// </seealso>
		/// <summary> Sets the concurrency level of the threading system of the Java Virtual
		...
		/// 
		/// </summary>
		/// <param name="n">The new concurrency level to set.
		/// 
		/// </param>
		/// <exception cref="IllegalArgumentException">Concurrency level is negative
		/// 
		/// </exception>
		/// <exception cref="UnsatisfiedLinkError">If the shared native library
		/// implementing the functionality could not be loaded.
		/// 
		/// </exception>
		public static int ThreadConcurrency
		{
			get
			{
			...
			set
			{
				...
				ThreadConcurrencyN = value;
			}
			
		}
		//UPGRADE_NOTE: Respective javadoc comments were merged.  It should be changed in order to comply with .NET documentation conventions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1199'"
		/// <summary> Calls the POSIX threads 'pthread_getconcurrency', or equivalent,
		...
		/// 
		/// </returns>
		/// <summary> Calls the POSIX threads 'pthread_setconcurrency', or equivalent,
		/// function with 'level' as the argument.
		/// 
		/// </summary>
		private static extern int ThreadConcurrencyN
		{
			get;
		...
		/// <summary>The name of the shared library containing the implementation of the
		...
		/// 
		/// </summary>
		public const System.String SHLIB_NAME = "jj2000";
		
		/// <summary>The state of the library loading </summary>
		private static int libState;
		
		/// <summary> Library load state ID indicating that no attept to load the library has
		/// been done yet.  
		/// </summary>
		private const int LIB_STATE_NOT_LOADED = 0;
		
		/// <summary> Library load state ID indicating that the library was successfully
		/// loaded. 
		/// </summary>
		private const int LIB_STATE_LOADED = 1;
		
		/// <summary> Library load state ID indicating that an attempt to load the library
		/// was done and that it could not be found. 
		/// </summary>
		private const int LIB_STATE_NOT_FOUND = 2;
		
		/// <summary> Private and only constructor, so that no class instance might be
		...
		{
			throw new System.ArgumentException("Class can not be instantiated");
		}
		
		/// <summary> Loads the shared library implementing the native methods of this
		...
		/// 
		/// </summary>
		/// <returns> True if the libary could be loaded or is already loaded. False
		...
		public static bool loadLibrary()
		{
			...
			try
			{
				...
				System_Renamed.loadLibrary(SHLIB_NAME);
			}
		...
		/// <summary> Checks if the library SHLIB_NAME is already loaded and attempts to load
		...
		/// 
		/// </summary>
		/// <exception cref="UnsatisfiedLinkError">If the library SHLIB_NAME can not be
		/// found.
		/// 
		/// </exception>
		private static void  checkLibrary()
		{
			switch (libState)
			{
				...
				case LIB_STATE_NOT_LOADED:  // Not yet loaded => load now
					...
					if (loadLibrary())
						break;
					goto case LIB_STATE_NOT_FOUND;
				
				case LIB_STATE_NOT_FOUND:  // Could not be found, throw exception
					//UPGRADE_NOTE: Exception 'java.lang.UnsatisfiedLinkError' was converted to 'System.Exception' which has different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1100'"
					throw new System.Exception("NativeServices: native shared " + "library could not be loaded");
				}
		}
	}
}